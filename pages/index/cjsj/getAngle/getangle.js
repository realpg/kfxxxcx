...
Page({
  ...
  viewTouchMove: function (e) {
    ...
    if (l[e.target.id] < -20 || t[e.target.id] < -20 || l[e.target.id] > vm.data.canvasWidth-20 || t[e.target.id] > vm.data.canvasHeight-20)
      return;
  ...
  draw:function(){
    ...
    if (length >= 1) {
      context.drawImage(vm.data.btn_path, vm.data.left[0], vm.data.top[0], 40, 40);
    ...
    for(var i=1;i<length;i++){  
      if(i ==1){
        context.moveTo(vm.data.left[0] + 20, vm.data.top[0] + 20);
      }
      

      context.lineTo(vm.data.left[i] + 20, vm.data.top[i] + 20)
      context.drawImage(vm.data.btn_path, vm.data.left[i], vm.data.top[i], 40, 40)
    ...
    //如果有三个点就计算角度，并显示
    if (length == 3){
      var x1 = vm.data.left[0]+20;
      var x2 = vm.data.left[1] + 20;
      var x3 = vm.data.left[2] + 20;
      var y1 = vm.data.top[0] + 20;
      var y2 = vm.data.top[1] + 20;
      var y3 = vm.data.top[2] + 20;
